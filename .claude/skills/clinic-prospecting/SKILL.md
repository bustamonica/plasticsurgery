...
- **Measure the crop per clinic; never transfer one.** Same defect, opposite
  ...
  height by measuring per height group - on all four of these it was pixels, but
  a mark can be scaled to the frame instead, or drawn proportional to the frame's
  width. AGENTS.md's four-crop bullet is the taxonomy of the crop mechanisms and
  which clinic uses which; pick by how the clinic draws its mark.
...
| Platform family is the highest-leverage question | One parser serves every clinic on the same site builder. The supported families and which clinic runs each are AGENTS.md's gallery-platform-families bullet and the `CLINICS` registry in `training/scripts/scrape_gallery.py` - read those rather than a copy, the roster grows with every batch | **Identify the builder before estimating integration cost.** A candidate on an already-supported family is worth far more than its case count suggests - it needs zero new parsing code, only a `CLINICS` config entry. View source and compare markup shape against `training/scripts/scrape_gallery.py`'s per-parser docstrings before assuming a new parser is needed |
