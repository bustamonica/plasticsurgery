...
export default function StudioPage() {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          config: { brandId, shape, profileId, volumeCc },
        }),
      });
  ...
  function downloadResult() {
    ...
    a.download = `preview-${brandId}-${shape}-${volumeCc}cc.png`;
    a.click();
  }
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      <div className="max-w-2xl">
        ...
        <p className="mt-3 text-ink-500">
          ...
          before &amp; after. Nothing is stored — everything happens in this session.
        </p>
      </div>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_400px]">
        ...
        <div>
          ...
          {!photo ? (
            <PhotoUpload onPhoto={(p) => { setPhoto(p); setResult(null); setError(null); }} />
          ) : (
            <div>
              {result ? (
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="space-y-6 rounded-2xl border border-cream-200 bg-white p-5 shadow-sm">
            ...
            <div>
              ...
              <select
                ...
                className="mt-2 w-full rounded-xl border border-cream-200 bg-white px-3 py-2.5 text-sm text-ink-900 focus:border-blush-400 focus:outline-none"
              >
                {PROFILES.map((p) => (
          ...
          <div className="rounded-2xl border border-cream-200 bg-white p-5 shadow-sm">
            ...
            <button
              ...
              className="mt-4 w-full rounded-full bg-blush-600 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blush-700 disabled:cursor-not-allowed disabled:bg-cream-300 disabled:text-ink-400"
            >
              {generating
                ? "Generating…"
                : result
            ...
            {!photo && (
              <p className="mt-2 text-center text-xs text-ink-400">Upload a photo first.</p>
            )}
            {error && (
              <p className="mt-3 rounded-xl bg-blush-50 px-4 py-3 text-sm text-blush-800">
                {error}
