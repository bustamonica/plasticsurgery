...
- Gallery intake: `training/scripts/scrape_gallery.py` (pluggable `CLINICS` configs; currently drkolker + drdanielbarrett-implants-only per captain scope, plus sanantonio BA-only). View laterality / unlabeled views come from a visual-annotation JSON (see the scraper docstring); `training/scripts/annotate_contact_sheets.py` renders the review sheets. Undocumented spec fields are omitted or the schema's `unknown` enum - never invented.
